...
export class AiProductGeneratorService {
    static async generateFromImage(imageBase64OrUrl, hintText, customApiKey) {
        return this.generateFromImages([imageBase64OrUrl], hintText, customApiKey);
    }
    static async generateFromImages(images, hintText, customApiKey) {
...
Return ONLY a single raw valid JSON object with this exact schema (no markdown triple backticks, no comments):
{
  ...
  "metaKeywords": ["handmade", "mirror-work", "festive-wear", "artisan-craft"]
    ...
    static sanitizeGeneratedData(data, defaultImage) {
        return {
            ...
            metaKeywords: Array.isArray(data.metaKeywords) ? data.metaKeywords : (data.keywords ? data.keywords.split(",").map((s) => s.trim()) : ["handmade", "latkan"])
        };
    }
    static generateDomainFallback(imageBase64OrUrl, hintText) {
