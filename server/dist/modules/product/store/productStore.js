...
class ProductStore {
    ...
    toSummary(p) {
        return {
            ...
            colors: p.colors || [],
            productType: p.productType || (p.variations && p.variations.length > 0 ? "variable" : "simple"),
            type: p.type || (p.variations && p.variations.length > 0 ? "Variable" : "Simple"),
            color: p.color || "",
            colorHex: p.colorHex || "",
    ...
    normalizeProduct(p) {
        ...
        const galleryImgs = Array.isArray(p.galleryImages) && p.galleryImages.length > 0
            ? p.galleryImages
            : allImages.filter((img) => img !== mainImg);
        ...
        return {
            ...
            colorHex: p.colorHex || "",
            size: p.size || "",
            productType: (p.productType === "simple" || p.type === "Simple") ? "simple" : (p.productType || (p.variations && p.variations.length > 0 ? "variable" : "simple")),
            type: (p.productType === "simple" || p.type === "Simple") ? "Simple" : (p.type || (p.variations && p.variations.length > 0 ? "Variable" : "Simple")),
            customAttributes: Array.isArray(p.customAttributes) ? p.customAttributes : [],
            colors: (p.productType === "simple" || p.type === "Simple")
                ? (p.color ? [{ id: `col-${p.id || 'main'}`, colorName: p.color, colorHex: p.colorHex || '#000000', displayImage: mainImg, mainImage: mainImg, galleryImages: allImages, sizes: [] }] : (Array.isArray(p.colors) && p.colors.length > 0 ? [p.colors[0]] : []))
                : (Array.isArray(p.colors) ? p.colors : []),
            variations: (p.productType === "simple" || p.type === "Simple") ? [] : (Array.isArray(p.variations) ? p.variations : (Array.isArray(p.variants) ? p.variants : [])),
            variants: (p.productType === "simple" || p.type === "Simple") ? [] : (Array.isArray(p.variants) ? p.variants : (Array.isArray(p.variations) ? p.variations : [])),
            ...
            reviewCount: p.reviewCount !== undefined && p.reviewCount !== null ? Number(p.reviewCount) : 0,
    ...
    async toggleStatus(id, customStatus) {
        ...
        let status = isPublished ? 'Published' : 'Inactive';
        if (customStatus) {
            status = customStatus;
