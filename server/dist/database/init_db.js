...
async function ensureColumnExists(connection, dbName, table, column, definition) {
    try {
        const [rows] = await connection.query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ?`, [dbName, table, column]);
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.warn("⚠️ CREATE DATABASE skipped (standard on cloud/shared hosting):", createErr.message);
            }
        ...
        console.warn(`⚠️ MySQL Database Auto-Initializer Note: ${error.message}`);
        return false;
    }
}
if (process.argv[1] &&
