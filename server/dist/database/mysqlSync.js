...
export async function syncProductToMySQL(p) {
    try {
        ...
        let finalSku = rawSku;
        try {
            const [existingSku] = await sequelize.query(`SELECT id FROM products WHERE default_sku = ? AND id != ? LIMIT 1`, { replacements: [rawSku, p.id] });
        ...
        try {
            const [existingSlug] = await sequelize.query(`SELECT id FROM products WHERE slug = ? AND id != ? LIMIT 1`, { replacements: [rawSlug, p.id] });
       ...
       ON DUPLICATE KEY UPDATE
        ...
        if (variations.length > 0) {
           ...
           ON DUPLICATE KEY UPDATE
            ...
            if (activeVariantIds.length > 0) {
                try {
                    await sequelize.query(`DELETE FROM product_variants WHERE product_id = ? AND id NOT IN (?)`, { replacements: [p.id, activeVariantIds] });
                }
                catch { }
            }
        }
            ...
            try {
                await sequelize.query(`DELETE FROM product_variants WHERE product_id = ?`, { replacements: [p.id] });
            }
            catch { }
        }
        // 7. Sync Images
        ...
        if (activeImgIds.length > 0) {
            try {
                await sequelize.query(`DELETE FROM product_images WHERE product_id = ? AND id NOT IN (?)`, { replacements: [p.id, activeImgIds] });
            }
            catch { }
        }
    }
