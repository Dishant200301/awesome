...
export async function fetchCategoriesFromMySQL(): Promise<{ categories: any[]; subcategories: any[] }> {
  try {
    ...
    try {
      ...
      console.warn("[MySQL Sync] Error calculating category product counts:", (err as Error).message);
    }
      ...
      if (isVariable) {
        for (const v of pVariants) {
          ...
          if (!colorsMap.has(cKey)) {
            const vImgs = Array.isArray(v.images) ? v.images.map((img: any) => typeof img === "string" ? img : img.url).filter(Boolean) : [];
...
export async function syncProductToMySQL(p: ProductItem): Promise<void> {
  try {
    ...
    if (Array.isArray(catRowsRes[0]) && catRowsRes[0].length > 0) {
      categoryId = (catRowsRes[0][0] as any).id;
    }
    if (Array.isArray(subRowsRes[0]) && subRowsRes[0].length > 0) {
      subcategoryId = (subRowsRes[0][0] as any).id;
    }
    ...
    const imageQueries: Promise<any>[] = [];
