...
const DEFAULT_SEED_MESSAGES: ContactMessage[] = [
  {
    id: "cm-101",
    name: "Priya Sharma",
...
let cachedContactMessages: ContactMessage[] = [...DEFAULT_SEED_MESSAGES];
let isTableInitialized = false;

export async function refreshContactMessagesFromMySQL(): Promise<ContactMessage[]> {
  try {
    ...
    if (Array.isArray(rows) && rows.length > 0) {
      cachedContactMessages = rows.map((r: any) => ({
        id: String(r.id),
    ...
    console.warn("[ContactStore] MySQL query failed, using in-memory cache:", (err as Error).message);
    return cachedContactMessages;
  }
}

export const getContactMessagesStore = async (filterStatus?: string, search?: string): Promise<ContactMessage[]> => {
...
export const getContactMessagesSync = (): ContactMessage[] => {
  return cachedContactMessages;
};

export const createContactMessageStore = async (data: Partial<ContactMessage>): Promise<ContactMessage> => {
  ...
  try {
    ...
    console.error("[ContactStore] Failed to insert contact message into MySQL:", (err as Error).message);
  }
...
export const updateContactMessageStatusStore = async (
  id: string,
  ...
  try {
    ...
    console.error(`[ContactStore] Failed to update contact status for '${id}':`, (err as Error).message);
  }
...
export const deleteContactMessageStore = async (id: string): Promise<boolean> => {
  try {
    await sequelize.query(`DELETE FROM contact_messages WHERE id = ?`, {
      replacements: [id]
    });
  } catch (err) {
    console.error(`[ContactStore] Failed to delete contact message '${id}':`, (err as Error).message);
  }
